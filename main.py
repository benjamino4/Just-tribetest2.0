...
# Owner-only key for the hidden control panel. Because a plain browser cannot
# supply Telegram's HMAC-signed initData, the owner authenticates the Forge
# with this secret (sent as X-Admin-Key / ?key=). Set it in the host env only.
...
async def require_admin_request(request: Request):
    """Authorize a Forge request two ways:
    1. A Telegram admin (HMAC-verified initData whose username/id is an elder).
    2. The owner's ADMIN_KEY secret (sent via X-Admin-Key header or ?key=),
       so the panel works from a plain browser where Telegram initData is
       unavailable. Never falls back to open access.
    """
    key = _admin_key_from(request)
    if ADMIN_KEY and hmac.compare_digest(key, ADMIN_KEY):
        return {"id": "owner", "username": "owner", "via": "key"}
    init_data = await _init_data_from(request)
    if init_data or not DEV_MODE:
        user = validate_init_data(init_data)
        require_admin(user)
        return user
    # DEV_MODE with no key configured: allow local testing only.
    if not ADMIN_KEY:
        return {"id": "dev-owner", "username": "owner", "via": "dev"}
    raise HTTPException(status_code=403, detail="forge is sealed")
...
async def api_found(request: Request):
    ...
    if not tname:
        raise HTTPException(status_code=400, detail="Name your tribe first")
    tc = cfg().get("tribe", {})
    ember_cost = int(tc.get("found_ember_cost", 0))
    with db() as conn:
        ...
        if int(u.get("ember", 0)) < ember_cost:
            raise HTTPException(
                status_code=400,
                detail=f"Founding a tribe costs {ember_cost} Ember - you have {int(u.get('ember', 0))}. Check in and finish quests to gather more.")
        tid = new_tribe_id(conn, tname)
        seed = int(cfg().get("loyalty", {}).get("found_seed", 100))
        if ember_cost:
            conn.execute("UPDATE users SET ember = ember - ? WHERE user_id = ?", (ember_cost, uid))
        conn.execute(
...
async def admin_overview(request: Request):
    """Everything the owner needs to steer the world: live tribes + kin rosters,
    current age, economy totals. Read-only snapshot for the control panel."""
    await require_admin_request(request)
    with db() as conn:
        tribes = _rows(conn.execute(
            "SELECT tribe_id, name, level, loyalty, embertide, chief_id FROM tribes ORDER BY loyalty DESC").fetchall())
        for t in tribes:
            t["members"] = tribe_member_count(conn, t["tribe_id"])
        users = _rows(conn.execute(
            "SELECT user_id, name, username, tribe_id, kindle, ember, wards, last_checkin FROM users ORDER BY kindle DESC LIMIT 200").fetchall())
        for u in users:
            u["state"] = activity_state(u.get("last_checkin", ""))
    return JSONResponse({
        "tribes": tribes, "users": users,
        "age": age_now(), "ages": list(cfg().get("age", {}).get("ages", {}).keys()),
        "dev_mode": DEV_MODE, "key_auth": bool(ADMIN_KEY),
    })
...
async def admin_tribe(request: Request):
    ...
    tid = str(body.get("tribe_id", ""))
    if not tid:
        raise HTTPException(status_code=400, detail="tribe_id required")
    with db() as conn:
        if not conn.execute("SELECT 1 FROM tribes WHERE tribe_id = ?", (tid,)).fetchone():
            raise HTTPException(status_code=404, detail="tribe not found")
...
async def _asset(name: str):
    # Serve only bundled image assets; block traversal.
    safe = os.path.basename(name)
    p = FRONTEND_DIR / "assets" / safe
    ext = safe.rsplit(".", 1)[-1].lower() if "." in safe else ""
    media = {"png": "image/png", "jpg": "image/jpeg", "jpeg": "image/jpeg",
             "webp": "image/webp", "svg": "image/svg+xml", "gif": "image/gif"}.get(ext)
    if not media or not p.exists():
        raise HTTPException(status_code=404, detail="not found")
    return FileResponse(str(p), media_type=media)


# --- Hidden owner-only control panel (the Elders' Forge). Not linked anywhere
# --- in the game UI; reachable only if you know the path. The page then asks
# --- for the ADMIN_KEY and every API call is authorized server-side.
@app.get("/forge")
async def _forge():
    return _static("admin.html", "text/html")


@app.get("/forge.js")
async def _forge_js():
    return _static("admin.js", "application/javascript")
