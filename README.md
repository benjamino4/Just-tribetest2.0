...
| `ADMIN_KEY` | Secret passphrase for the owner control panel | required to open the Forge from a browser |
...
## Elders' Forge \u2014 hidden owner control panel

The admin panel is **not** shown anywhere inside the game (no Forge tab). It
lives at a separate, unlinked URL: `https://<your-app>/forge`

Open that URL in any browser, enter your `ADMIN_KEY`, and you get full control:
switch the world Age, retune every economy number (Ember/Kindle/Loyalty/Ashfall/
Rekindle/tribe founding cost/referrals/lands), add-edit-remove quests and store
items (incl. Star prices), grant Kindle/Ember/Loyalty/Embertide, rename/retune/
disband tribes, run Ashfall on demand, set the bot username, edit the raw config
override, or reset to defaults. Every change persists in the DB.

Auth is server-enforced: the Forge accepts either a Telegram admin
(HMAC-verified) **or** the `ADMIN_KEY` secret (sent as an `X-Admin-Key` header).
A plain browser can't produce Telegram's signed `initData`, so the key is what
lets **you** \u2014 and only you \u2014 in. Keep `ADMIN_KEY` long and private.
...
- Env vars: `DATABASE_URL`, `BOT_TOKEN`, `ADMIN_KEY`
